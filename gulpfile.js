...
gulp.task('twig', function (done) {
    return gulp.src('src/_view-markup-modal.twig')
        .pipe(twig())
...
gulp.task('js', function (done) {
    ...
    // Save css string
    let cssFileContent = fs.readFileSync('dist/temp/view-markup.css', 'utf8');
    
    // Save html string
    let modalFileContent = fs.readFileSync('dist/temp/_view-markup-modal.html', 'utf8');
        
    return gulp.src([`${roots.src}/js/view-markup.js`], { sourcemaps: true })
        .pipe(include())
...
gulp.task('sass', function (done) {
    ...
    return gulp.src([`${roots.src}/scss/view-markup.scss`])
